...
temp/u2u_%.csv: temp/%.parquet src/project/u2u.sql
	...
	duckdb < temp.sql
temp/editors.parquet: src/filter/editors.sql temp/ghtorrent.db data/users.csv 
	duckdb -readonly temp/ghtorrent.db < $<
temp/organizations.parquet: src/filter/organizations.sql temp/ghtorrent.db data/users.csv 
	duckdb -readonly temp/ghtorrent.db < $<
temp/%.parquet: src/filter/%.sql temp/events.db data/users.csv data/projects.csv
	duckdb -readonly temp/events.db < $<
data/users.csv data/projects.csv &: src/filter/export_sample.sql temp/ghtorrent.db 
	duckdb -readonly temp/ghtorrent.db < $<